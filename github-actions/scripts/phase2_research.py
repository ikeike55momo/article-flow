#!/usr/bin/env python3
"""Phase 2: Research - Conduct comprehensive web research with parallel processing"""
...
from utils.web_search import BingSearchAPI, ParallelSearcher


def parse_arguments():
    """Parse command line arguments"""
    parser = argparse.ArgumentParser(description="Phase 2: Research")
    parser.add_argument("--params-file", required=True, help="Phase 1 output JSON file")
    parser.add_argument("--output-dir", required=True, help="Output directory")
    parser.add_argument("--parallel-batches", type=int, default=5, help="Number of parallel search batches")
...
def process_search_batch(
    ...
    for query in queries:
        try:
            ...
            results = searcher.search_with_priority(query, count=10)
...
def parallel_research(
    ...
    """Execute research queries in parallel batches"""
...
def extract_key_sources(research_data: Dict[str, Any], claude: ClaudeAPI) -> Dict[str, Any]:
    """Extract and analyze key sources from research results"""
    ...
    for search_result in research_data["search_results"]:
        for result in search_result.get("results", []):
            if result.get("priority") in ["very_high", "high"]:
    ...
    # Have Claude analyze and categorize sources
    analysis_prompt = f"""
    以下の高優先度ソースを分析し、カテゴリ別に整理してください：
    
    {json.dumps(high_priority_sources, ensure_ascii=False, indent=2)}
    
    以下の形式でJSONとして出力してください：
    {{
        "government_sources": [{{"url": "", "title": "", "key_info": ""}}],
        "academic_sources": [{{"url": "", "title": "", "key_info": ""}}],
        "medical_sources": [{{"url": "", "title": "", "key_info": ""}}],
        "industry_sources": [{{"url": "", "title": "", "key_info": ""}}],
        "media_sources": [{{"url": "", "title": "", "key_info": ""}}]
    }}
    """
    
    categorized_sources = claude.generate_with_structured_output(
        prompt=analysis_prompt,
        system_prompt="You are a research analyst specializing in source evaluation and categorization.",
        temperature=0.3,
        metadata={"phase": "source_categorization"}
    )
...
def main():
    ...
    # Setup logging
    logger = setup_logging("phase2_research", args.log_level)
    log_phase_start(logger, "Phase 2: Research")
    ...
    try:
        ...
        # Extract key sources
        source_analysis = extract_key_sources(research_data, claude)
        ...
        log_phase_end(logger, "Phase 2: Research", success=True)
        
    except Exception as e:
        log_error(logger, e, "Phase 2")
        log_phase_end(logger, "Phase 2: Research", success=False)
        sys.exit(1)
